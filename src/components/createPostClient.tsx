...
const CreatePostClient: React.FC<CreatePostClientProps> = ({ user }) => {
      ...
      <div className="text-center">
        <div className="flex flex-col items-center justify-center mb-4 p-4 dark:bg-black bg-white rounded shadow-lg w-[500px] mx-auto">
          <div className="flex items-center w-full mb-4">
            ...
            <div className="ml-4 text-xl">{user.name || "Unknown User"}</div>
          </div>
          <form className="flex flex-col" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              className="mt-4 bg-blue-500 text-white py-2 px-6 rounded-md hover:bg-blue-700 disabled:bg-blue-300"
              disabled={buttonDisabled}
            >
              Post
            </button>
          </form>
          {statusMessage && <div className="text-sm mt-2">{statusMessage}</div>}
        </div>
      </div>
    </div>
  );
